...
export class QuotationItemComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.quotationItemService
            ...
            .subscribe(
                (res: HttpResponse<IQuotationItem[]>) => this.paginateQuotationItems(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/quotation-item'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: IQuotationItem) {
        return item.id;
    }
    ...
    private paginateQuotationItems(data: IQuotationItem[], headers: HttpHeaders) {
